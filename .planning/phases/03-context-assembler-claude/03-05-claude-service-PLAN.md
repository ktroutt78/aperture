...
- **03-02** for `StreamParser` (chunk-boundary-robust parser)
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - backend/src/services/streamParser.ts (from Plan 03-02 — the class interface streamChat pipes into)
  ...
  <behavior>
    ...
    10. **Usage passthrough** — done event's `usage` field pulls inputTokens/outputTokens/cacheReadTokens from the SDK's final message (or falls back to 0 if the stub doesn't provide them).
  </behavior>
  <action>
...
export async function* streamChat(
  ...
  if (!env.anthropicApiKey && !deps.client) {
    yield {
      type: 'error' as const,
      data: { code: 'INTERNAL' as ErrorCode, message: 'ANTHROPIC_API_KEY not set' },
    } as unknown as StreamParserEvent;
    return;
  }
  ...
  try {
    ...
    for await (const event of stream as AsyncIterable<{ type?: string; delta?: { text?: string }; message?: { stop_reason?: string; usage?: Record<string, number> } }>) {
      if (event.type === 'content_block_delta' && event.delta?.text) {
        parser.feed(event.delta.text);
      } else if (event.type === 'message_delta' && event.message?.stop_reason) {
    ...
    yield {
      type: 'error' as const,
      data: { code, message: (err as Error).message },
    } as unknown as StreamParserEvent;
...
**NOTE on error events:** Plan 03-02's StreamParser does NOT define an `error` event in its type union. Extend the union here by yielding `error` / `done` events with `as unknown as StreamParserEvent` cast, OR update `StreamParser`'s exported `StreamParserEvent` type to include `{ type: 'error'; data: { code: ErrorCode; message: string } }`. Executor's call — document the chosen approach in the SUMMARY. Recommended: extend `StreamParserEvent` in Plan 03-02's file via a small follow-up edit during this task (keeps the type clean).
...
- Test 9 (missing API key) must call `__resetEnvCacheForTests()` and temporarily unset `process.env.ANTHROPIC_API_KEY` via `delete process.env.ANTHROPIC_API_KEY`. Restore after.
  ...
  <done>streamChat yields typed events from a stubbed SDK, model is locked, history is capped, errors are mapped to the stable ErrorCode enum, no API key needed for offline tests.</done>
</task>
